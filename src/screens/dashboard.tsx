...
import Bukidnon from './../assets/bukidnon.png'
import Camiguin from './../assets/camiguin.png'
import LanaoDelSur from './../assets/lanao.png'
import MisamisOccidental from './../assets/misoc.png'
  ...
  const renderRequirementsTable = (rows: RequirementItem[]) => (
    <div className="w-full text-sm rounded-lg overflow-hidden border border-blue-200">
      <div className="grid grid-cols-[3fr_1fr]">
        <div className="bg-blue-600 text-white font-semibold p-2">CHECKLIST OF REQUIREMENTS</div>
        <div className="bg-blue-600 text-white font-semibold p-2 border-l border-blue-500">WHERE TO SECURE</div>
      </div>
      {rows.map((r, i) => (
        <div key={i} className={`grid grid-cols-[3fr_1fr] ${i % 2 === 0 ? "bg-white" : "bg-blue-50"}`}>
          <div className="p-2 text-slate-700 border-t border-r border-blue-100 leading-relaxed" dangerouslySetInnerHTML={{ __html: sanitizeHtml(r.checklist_of_requirements) }} />
          <div className="p-2 text-slate-600 border-t border-blue-100 leading-relaxed" dangerouslySetInnerHTML={{ __html: sanitizeHtml(r.where_to_secure) }} />
        </div>
      ))}
    </div>
  )
    ...
    return (
      <div className="space-y-4">
        {Object.entries(reqs).map(([k, items]) => (
          <div key={k}>
            <p className="text-sm font-semibold text-blue-700 capitalize mb-1">{k.replace(/_/g, " ")}</p>
    ...
    >
      ...
      <div className="relative flex items-center gap-5 p-5">
        ...
        <div className="flex-shrink-0">
          <div className="w-40 h-40 rounded-full overflow-hidden shadow-xl flex items-center justify-center bg-blue-800">
            {po.photo
              ? <img src={po.photo} alt={po.name} className="w-full h-full object-cover" />
              : <svg className="w-12 h-12 text-white/60" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z" /></svg>
            }
          </div>
        </div>
                    ...
                    filteredServices.map((item, index) => (
                      ...
                      >
                        ...
                        <div className="text-left relative z-10">
                          <h3 className="text-2xl font-bold text-slate-800 group-hover:text-blue-700 transition-colors">{renderHtml(item.title)}</h3>
 ...
 <div className="mt-8 bg-gradient-to-br from-yellow-50 to-orange-50 border-l-4 border-yellow-400 rounded-xl shadow p-6">
                  ...
                  <div className="text-slate-700 text-sm space-y-3">
                    <div>
                      ...
                      <ul className="list-disc ml-6 mt-1 space-y-1">{charterData.document.feedback_and_complaints_mechanism.how_to_send_feedback.map((item, i) => (<li key={i}>{renderHtml(item)}</li>))}</ul>
                    </div>
                    <div>
                      <span className="font-semibold text-yellow-700">How Feedback Is Processed:</span>
                      <ol className="list-decimal ml-6 mt-1 space-y-1">{charterData.document.feedback_and_complaints_mechanism.how_feedbacks_are_processed.map((item, i) => (<li key={i}>{renderHtml(item)}</li>))}</ol>
                    </div>
                    <div>
                      <span className="font-semibold text-yellow-700">How to File a Complaint:</span>
                      <ul className="list-disc ml-6 mt-1 space-y-1">
                        <li>{renderHtml(charterData.document.feedback_and_complaints_mechanism.how_to_file_a_complaint.channel)}</li>
                        <li>Required details:<ul className="list-[circle] ml-6 mt-1">{charterData.document.feedback_and_complaints_mechanism.how_to_file_a_complaint.required_details.map((d, i) => (<li key={i}>{renderHtml(d)}</li>))}</ul></li>
                        <li>Other channels:<ul className="list-[circle] ml-6 mt-1">{charterData.document.feedback_and_complaints_mechanism.how_to_file_a_complaint.other_channels.map((ch, i) => (<li key={i}><b>{ch.name}</b>{"email" in ch && <span>: <span style={{ color: "#1d4ed8" }}>{ch.email}</span></span>}{"hotline" in ch && <span> / Hotline: <b>{ch.hotline}</b></span>}{"contact" in ch && <span>: <b>{ch.contact}</b></span>}</li>))}</ul></li>
                      </ul>
                    </div>
                    <div>
                      <span className="font-semibold text-yellow-700">How Complaints Are Processed:</span>
                      <ol className="list-decimal ml-6 mt-1 space-y-1">{charterData.document.feedback_and_complaints_mechanism.how_complaints_are_processed.map((item, i) => (<li key={i}>{renderHtml(item)}</li>))}</ol>
                    </div>
                    <div>
                      <span className="font-semibold text-yellow-700">Contact Information:</span>
                      <ul className="list-disc ml-6 mt-1">{Object.entries(charterData.document.feedback_and_complaints_mechanism.contact_information_of_ccb_pcc_arta).map(([k, v]) => (<li key={k}>{k}: {v}</li>))}</ul>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 flex flex-col p-6 overflow-hidden animate-fade-in">
                <Button onClick={handleBackToList} variant="ghost" className="w-fit mb-6 text-xl text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-lg group">
                ...
                <div ref={detailScrollRef} className="flex-1 overflow-y-auto custom-scrollbar space-y-5">
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-6 rounded-xl border-l-4 border-blue-500 shadow-sm space-y-5">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-white rounded-lg p-3 border border-blue-100"><p className="text-xs font-semibold text-blue-500 uppercase tracking-wide mb-1">Office / Division</p><p className="text-sm text-slate-700">{renderHtml(selectedService.office_or_division)}</p></div>
                      <div className="bg-white rounded-lg p-3 border border-blue-100"><p className="text-xs font-semibold text-blue-500 uppercase tracking-wide mb-1">Classification</p><p className="text-sm text-slate-700">{renderHtml(selectedService.classification)}</p></div>
                      <div className="bg-white rounded-lg p-3 border border-blue-100"><p className="text-xs font-semibold text-blue-500 uppercase tracking-wide mb-1">Type of Transaction</p><div className="flex flex-col gap-1">{selectedService.type_of_transaction.map((t, i) => (<p key={i} className="text-sm text-slate-700">{renderHtml(t)}</p>))}</div></div>
                      <div className="bg-white rounded-lg p-3 border border-blue-100"><p className="text-xs font-semibold text-blue-500 uppercase tracking-wide mb-1">Who May Avail</p><p className="text-sm text-slate-700">{renderHtml(selectedService.who_may_avail)}</p></div>
                    </div>
                    <div>
                      <h3 className="text-xl font-bold text-slate-800 mb-2 flex items-center gap-2"><div className="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center"><FileText className="w-4 h-4 text-white" /></div>Description</h3>
                      <p className="text-slate-700 text-base leading-relaxed">{renderHtml(selectedService.description)}</p>
                    </div>
                    <div>
                      <h4 className="font-semibold text-xl text-blue-700 mb-2">Checklist of Requirements:</h4>
                      {renderRequirements(selectedService.checklist_of_requirements)}
                    </div>
                    <div>
                      <h4 className="font-semibold text-xl text-blue-700 mb-2">Process Flow:</h4>
                      <div className="w-full text-sm rounded-lg overflow-hidden border border-blue-200">
                        <div className="grid grid-cols-[2fr_2fr_1fr_1fr_2fr]">
                          <div className="bg-blue-600 text-white font-semibold p-2">CLIENT STEPS</div>
                          <div className="bg-blue-600 text-white font-semibold p-2 border-l border-blue-500">AGENCY ACTION</div>
                          <div className="bg-blue-600 text-white font-semibold p-2 border-l border-blue-500">FEES TO BE PAID</div>
                          <div className="bg-blue-600 text-white font-semibold p-2 border-l border-blue-500">PROCESSING TIME</div>
                          <div className="bg-blue-600 text-white font-semibold p-2 border-l border-blue-500">PERSON RESPONSIBLE</div>
                        </div>
                        {selectedService.process_flow.map((step, idx) => (
                          <div key={idx} className={`grid grid-cols-[2fr_2fr_1fr_1fr_2fr] ${idx % 2 === 0 ? "bg-white" : "bg-blue-50"}`}>
                            <div className="p-2 text-slate-700 border-t border-r border-blue-100 leading-relaxed" dangerouslySetInnerHTML={{ __html: sanitizeHtml(step.client_steps ?? "") }} />
                            <div className="p-2 text-slate-700 border-t border-r border-blue-100 leading-relaxed" dangerouslySetInnerHTML={{ __html: sanitizeHtml(step.agency_action) }} />
                            <div className="p-2 text-slate-700 border-t border-r border-blue-100 leading-relaxed" dangerouslySetInnerHTML={{ __html: sanitizeHtml(step.fees_to_be_paid) }} />
                            <div className="p-2 text-slate-700 border-t border-r border-blue-100 leading-relaxed" dangerouslySetInnerHTML={{ __html: sanitizeHtml(step.processing_time) }} />
                            <div className="p-2 text-slate-700 border-t border-blue-100 leading-relaxed" dangerouslySetInnerHTML={{ __html: sanitizeHtml(step.person_responsible) }} />
                          </div>
                        ))}
                      </div>
                    </div>
                    {selectedService.total && (
                      <div className="flex gap-4">
                        <div className="flex-1 bg-white rounded-lg p-3 border border-blue-100"><p className="text-xs font-semibold text-blue-500 uppercase tracking-wide mb-1">Total Fees to be Paid</p><p className="text-base font-bold text-slate-800">{renderHtml(selectedService.total.fees_to_be_paid)}</p></div>
                        <div className="flex-1 bg-white rounded-lg p-3 border border-blue-100"><p className="text-xs font-semibold text-blue-500 uppercase tracking-wide mb-1">Total Processing Time</p><p className="text-base font-bold text-slate-800">{renderHtml(selectedService.total.processing_time)}</p></div>
                      </div>
                    )}
                    {selectedService.mode_of_payment && (
                      <div className="bg-white rounded-lg p-3 border border-blue-100"><p className="text-xs font-semibold text-blue-500 uppercase tracking-wide mb-1">Mode of Payment</p><p className="text-sm text-slate-700">{renderHtml(selectedService.mode_of_payment)}</p></div>
                    )}
                    {selectedService.regional_email_contacts && (
                      <div>
                        <h4 className="font-semibold text-xl text-blue-700 mb-2">Regional Email Contacts:</h4>
                        <div className="overflow-x-auto">
                          <table className="w-full text-sm border-collapse">
                            <thead><tr className="bg-blue-600 text-white"><th className="p-2 text-left font-semibold w-1/3">Region</th><th className="p-2 text-left font-semibold w-2/3">Email Address</th></tr></thead>
                            <tbody>
                              {Object.entries(selectedService.regional_email_contacts).map(([region, email], i) => (
                                <tr key={region} className={i % 2 === 0 ? "bg-white" : "bg-blue-50"}>
                                  <td className="p-2 align-top text-slate-700 font-medium border border-blue-100">{region}</td>
                                  <td className="p-2 align-top border border-blue-100">
                                    {Array.isArray(email)
                                      ? <div className="flex flex-col gap-1">{email.map((e, idx) => (<a key={idx} href={`mailto:${e}`} className="text-blue-600 hover:underline">{e}</a>))}</div>
        ...
        <div className="w-[30%] flex flex-col gap-4">
          ...
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl border border-blue-100/50 p-5 animate-fade-in-down overflow-hidden relative">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                {currentPanel === 0 ? (<><div className="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center"><Calendar className="w-4 h-4 text-white" /></div>Events</>) : currentPanel === 1 ? (<><div className="w-8 h-8 bg-emerald-500 rounded-lg flex items-center justify-center"><Newspaper className="w-4 h-4 text-white" /></div>News</>) : (<><div className="w-8 h-8 bg-orange-500 rounded-lg flex items-center justify-center"><Megaphone className="w-4 h-4 text-white" /></div>Announcements</>)}
              </h2>
              <div className="flex gap-1.5">
                <button onClick={toggleAutoSwipe} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-300 ${!isAutoSwipe ? "bg-blue-500 text-white shadow-md" : "bg-slate-100 text-slate-400 hover:bg-slate-200"}`}>
                ...
                <div className="w-full flex-shrink-0 relative">
                  <div ref={eventsScrollRef} className="space-y-2.5 max-h-[200px] overflow-y-auto custom-scrollbar" onScroll={handleEventsScroll}>
                    {events.slice(0, 4).map((event, index) => (
                      <div key={index} onClick={() => setSelectedEvent(event)} className="group cursor-pointer bg-gradient-to-r from-blue-50 to-white p-4 rounded-lg border-l-4 border-blue-500 hover:shadow-md transition-all duration-300 hover:-translate-y-0.5">
                        <h3 className="text-sm font-bold text-slate-800 group-hover:text-blue-700 mb-2">{event.title}</h3>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="px-2.5 py-1 bg-blue-500 text-white rounded-lg text-xs font-semibold">{event.time}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                  {showEventsScrollHint && currentPanel === 0 && (<div className="absolute bottom-0 left-0 right-0 h-12 bg-gradient-to-t from-white via-white/80 to-transparent pointer-events-none flex items-end justify-center pb-1"><div className="animate-bounce-slow"><svg className="w-6 h-6 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" /></svg></div></div>)}
                </div>
                {/* News */}
                <div className="w-full flex-shrink-0 relative">
                  <div ref={newsScrollRef} className="space-y-2.5 max-h-[200px] overflow-y-auto custom-scrollbar" onScroll={handleNewsScroll}>
                    {news.map((item, index) => (
                      <div key={index} onClick={() => setSelectedNews(item)} className="group cursor-pointer bg-red-500 rounded-lg border border-emerald-200 hover:shadow-lg transition-all overflow-hidden relative flex">
                  ...
                  {showNewsScrollHint && currentPanel === 1 && (<div className="absolute bottom-0 left-0 right-0 h-12 bg-gradient-to-t from-white via-white/80 to-transparent pointer-events-none flex items-end justify-center pb-1"><div className="animate-bounce-slow"><svg className="w-6 h-6 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" /></svg></div></div>)}
                </div>
                {/* Announcements */}
                <div className="w-full flex-shrink-0 relative">
          ...
          {/* Video */}
          <div className="flex-1 min-h-0 bg-white backdrop-blur-sm rounded-2xl shadow-xl border border-blue-100/50 p-4 pb-2 flex flex-col animate-fade-in-up">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {videoCountdown > 0 && <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-lg text-xs font-semibold">{Math.floor(videoCountdown / 60)}:{String(videoCountdown % 60).padStart(2, "0")}</span>}
                </div>
              </div>
            ...
            <div className="flex items-center justify-center gap-2 mt-2">
              {videos.map((_, index) => (
                <button key={index} onClick={() => setCurrentVideoIndex(index)} className={`group relative transition-all duration-300 ${index === currentVideoIndex ? "w-10" : "w-2.5"}`}>
                  <div className={`h-2 rounded-full transition-all duration-300 ${index === currentVideoIndex ? "bg-gradient-to-r from-blue-500 to-indigo-600 shadow-lg" : "bg-slate-300 group-hover:bg-slate-400"}`} />
                  {index === currentVideoIndex && <div className="absolute inset-0 bg-blue-400 rounded-full animate-ping opacity-20" />}
                </button>
              ))}
            </div>
          </div>
      ...
      {mapModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-6 animate-fade-in" onClick={() => setMapModal(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden animate-fade-in-up" onClick={(e) => e.stopPropagation()}>
            {/* Header */}
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-4 flex items-center justify-between gap-3">
      ...
      {showKeyboard && (
        <div className="fixed inset-0 bg-black/30 z-50 flex items-end justify-center animate-fade-in" onClick={() => setShowKeyboard(false)}>
          <div className="bg-white w-full max-w-4xl rounded-t-3xl shadow-2xl p-6 animate-slide-up" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2"><Search className="w-5 h-5 text-blue-600" />Virtual Keyboard</h3>
            ...
            <div className="space-y-2">
              <div className="flex gap-2 justify-center">{["Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P"].map((k) => (<button key={k} onClick={() => handleKeyPress(k.toLowerCase())} className="w-12 h-12 bg-gradient-to-br from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-bold rounded-lg shadow-md active:scale-95 transition-all">{k}</button>))}</div>
              <div className="flex gap-2 justify-center">{["A", "S", "D", "F", "G", "H", "J", "K", "L"].map((k) => (<button key={k} onClick={() => handleKeyPress(k.toLowerCase())} className="w-12 h-12 bg-gradient-to-br from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-bold rounded-lg shadow-md active:scale-95 transition-all">{k}</button>))}</div>
              <div className="flex gap-2 justify-center">{["Z", "X", "C", "V", "B", "N", "M"].map((k) => (<button key={k} onClick={() => handleKeyPress(k.toLowerCase())} className="w-12 h-12 bg-gradient-to-br from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-bold rounded-lg shadow-md active:scale-95 transition-all">{k}</button>))}</div>
